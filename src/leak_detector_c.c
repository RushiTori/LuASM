 ...
 */
void *xrealloc(void *mem_ref, unsigned int size, const char *file,
               unsigned int line) {
  ...
  if (ptr != NULL) {
    ...
    add_mem_info(mem_ref, size, file, line);
  }
  return ptr;
}
...
void add_mem_info(void *mem_ref, unsigned int size, const char *file,
  ...
  strcpy(mem_alloc_info.file_name,
         file /*,__min(FILE_NAME_LENGTH, strlen(file))*/);
